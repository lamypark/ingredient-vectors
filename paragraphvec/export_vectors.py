...
def start(data_file_name, model_file_name, use_embeddings=False):
    ...
    """
    dataset, embeddings = load_dataset(data_file_name, use_embeddings)
...
def _load_model(model_file_name, vec_dim, embeddings, num_docs, num_words):
    ...
    if model_ver == 'dbow':
        model = DBOW(vec_dim, num_docs, num_words, embeddings)
    else:
        model = DM(vec_dim, num_docs, num_words, embeddings)
