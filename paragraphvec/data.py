...
def load_dataset(file_name, use_embeddings=False):
    ...
    label_field = Field(pad_token=None)
    ...
    print(dataset[0].text, dataset[0].label)
    ...
    embeddings = {}
    if use_embeddings:
        file_name_embeddings = "flavordb/compounds_info.pkl"
        file_path_embeddings = join(DATA_DIR, file_name_embeddings)
        with open(file_path_embeddings, 'rb') as f:
            compounds_info = pickle.load(f)

        for compound in compounds_info:
            _id = compounds_info[compound]['comp_index_id']
            _vect = compounds_info[compound]['comp_mol2vec']

            embeddings[_id] = _vect

        embeddings = pd.DataFrame(embeddings).T.values
...
class _NCEGenerator(object):
    ...
    def next(self):
        ...
        #print(prev_doc_id, prev_in_doc_pos)
