...
def start(data_file_name,
    ...
    if use_embeddings:
        dataset, embeddings = load_dataset(data_file_name, use_embeddings=True)
    else:
        dataset, embeddings = load_dataset(data_file_name, use_embeddings=False)
    ...
    try:
        _run(data_file_name, dataset, embeddings, nce_data.get_generator(), len(nce_data),
             nce_data.vocabulary_size(), context_size, num_noise_words, vec_dim,
             num_epochs, batch_size, lr, model_ver, vec_combine_method,
             save_all, generate_plot, model_ver_is_dbow, use_embeddings)
...
def _run(data_file_name,
    ...
    if model_ver_is_dbow:
        model = DBOW(vec_dim, num_docs=len(dataset), num_words=vocabulary_size, embeddings=embeddings)
    else:
        model = DM(vec_dim, num_docs=len(dataset), num_words=vocabulary_size, embeddings=embeddings)
    ...
    for epoch_i in range(num_epochs):
        ...
        best_loss = loss
        is_best_loss = loss <=  float(best_loss)
