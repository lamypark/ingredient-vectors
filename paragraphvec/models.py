...
class DM(nn.Module):
    ...
    """
    def __init__(self, vec_dim, num_docs, num_words, embeddings):
        ...
        if embeddings is not None:
            # input with pre-trained embeddings
            self._W = nn.Parameter(
                torch.from_numpy(embeddings), requires_grad=True).type(torch.cuda.FloatTensor)

        else:
            # input randomly initialized
            self._W = nn.Parameter(
...
class DBOW(nn.Module):
    ...
    """
    def __init__(self, vec_dim, num_docs, num_words, use_embeddings):
